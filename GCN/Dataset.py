...
class Twibot20 (Dataset):
    def __init__(self,root='./Data/',device='cpu',process=False,save=False):
        self.root = root
        self.device = device
    ...
    def prop_caucu_ig(self):
        path_0=self.root+'num_properties_tensor.pt'
        path_1=self.root+'cat_properties_tensor.pt'
        
        num_prop_tensor=torch.load(path_0).to(self.device)
        cat_prop_tensor=torch.load(path_1).to(self.device)
        y=torch.load(self.root+'label.pt').to(self.device)
        mi_num=mic(num_prop_tensor,y)
        mi_cat=mic(cat_prop_tensor,y)

        return mi_num,mi_cat
